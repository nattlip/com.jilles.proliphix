...
class NT20Device extends Homey.Device {
    ...
    onInit() {
        ...
        this.thermostatGetCommand = `/get?${this.thermostatTempOneCommand}=&${this.thermostatTempTwoCommand}=&${this.thermostatTempThreeCommand}\
=&${this.thermostatThermSetbackHeatCommand}=&${this.thermostatThermHvacStateCommand}=` // = path in req
        ...
        let callback2 =  (response) => {
           let result = '';
            ...
            response.on('end',  () =>  {
                ...
                if (method == 'GET') {
                    // temp1
                    let is = result.indexOf(`OID4.3.2.1=`)
                    let strippedresult = result.slice(is + 1 + 10, is + 14)
                    util.log(`strippedresult req`, strippedresult)
                    let answer = parseInt(strippedresult)
                    util.log(`int strippedresult `, parseInt(strippedresult))
                    let temp = (((answer / 10) - 32) / 1.8).toFixed(2)
                    util.log(`temperature 1`, `  ${temp}  Celsius`)
                    this.setCapabilityValue(`measure_temperature_one`, Number(temp))

                    // temp 2
                    let is1 = result.indexOf(`OID4.3.2.2=`)
                    let strippedresult1 = result.slice(is1 + 1 + 10, is1 + 14)
                    util.log(`strippedresult1 req`, strippedresult1)
                    //if (strippedresult1[0] == "-")
                    //{ strippedresult1[0] = 0 } 
                    let answer1 = parseInt(strippedresult1)
                    util.log(`int strippedresult1 `, parseInt(strippedresult1))
                    let temp1 = (((answer1 / 10) - 32) / 1.8).toFixed(2)
                    util.log(`temperature 1`, `  ${temp1}  Celsius`)
                    this.setCapabilityValue(`measure_temperature_two`, Number(temp1))

                    // temp 3
                    let is2 = result.indexOf(`OID4.3.2.3=`)
                    let strippedresult2 = result.slice(is2 + 1 + 10, is2 + 14)
                    ...
                    util.log(`temperature`, `  ${temp2}  Celsius`)
